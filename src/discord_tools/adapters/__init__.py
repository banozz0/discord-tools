...
Five are implemented: the three the output contract needs — who the run acts
as, what it acts on, and what rights it holds there — the archive source the
local archive fills from, and the media fetcher the review queue's approved
downloads stream through. The rest of the Protocols in `_core.adapters`
...
__all__ = ["DiscordArchiveSource", "DiscordIdentityProvider", "DiscordMediaFetcher", "DiscordPermissionProbe", "DiscordTargetResolver"]
