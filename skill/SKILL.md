...
object as the last line, marked `"kind": "envelope"`. Without `--json`, `send`
and the `message` verbs end on one sentence for a person (`Sent message <id> to
#<channel> (<channel id>).`) and print no mapping at all: the ids are in
`result`, so pass `--json` rather than scraping that line.
