...
Six are implemented: the three the output contract needs — who the run acts
as, what it acts on, and what rights it holds there — the archive source the
local archive fills from, the media fetcher the review queue's approved
downloads stream through, and the blueprint port structure export, diff and
apply go through. The rest of the Protocols in `_core.adapters` belong to
later capability packs and are deliberately absent — a Protocol with no caller
is a guess about a signature, and the shared tree says the card that lands one
settles it.
"""
...
from discord_tools.adapters.blueprint import DiscordBlueprintPort
from discord_tools.adapters.identity import DiscordIdentityProvider
from discord_tools.adapters.media import DiscordMediaFetcher
